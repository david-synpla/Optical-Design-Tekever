...
Stage: C1 COMPLETE through Run 021 cross-portfolio closure; C01 and C02 advance to a gated two-architecture C2 comparison, while C04 remains a procurement-risk hedge
...
|---|---|---|---|---|
| C01 | Separate optics on an integrated common bench/shell | ADVANCE to C2 as preferred reference/fallback; B05-1/S16 baseline, S20 performance and S14 stretch retained | Proven B05 EO reuse, compact shared structure, 90–95% traced SWIR pupil survival, no splitter/complex SWIR camera | Four custom conic mirrors; real common-bench/thermal/gimbal mass and relative-boresight stability remain gating evidence |
| C02 | Common coaxial reflective fore-telescope plus independent EO/SWIR relays | CONDITIONALLY ADVANCE to the C2 gate through C02-A6F1; direct form and two-element backends PARKED | Curved-field common front is near its matched obscured-pupil diffraction limit; one frontal aperture/common LOS | Quoteable annular-primary process, finite dichroic transition and real three-element cameras/fold are all still blocking a sole C02 commitment |
...
| C04 | Cost/COTS-maximized dual-channel branch | RETAIN as procurement-risk hedge through C04-N1400/B09; do not advance to C2 optical development on current evidence | Catalogue 353/1600 large powered figure plus stock flat and only two small custom spherical SWIR elements; exact first order and ~9.22 um dense SWIR RMS | Supplier answers can validate economics but cannot alone cure the 273 L / 1.455 m / 8.28 kg-primary UAV burden |
...
No Track C candidate has been frozen or selected as the sole product architecture. Run 021 closes C1 with a two-architecture C2 gate; it is an RFQ-readiness analysis, not detailed tolerancing, CAD, a supplier quotation or a production-cost model. Run 016's physical C02 doublets remain parked. Run 014 package ranges and ordinal risk classes remain engineering hypotheses, not customer requirements, CAD results or quotations.
...
## C1 closure and C2 decision gate — Run 021

Run 021 normalized C01 B05-1/S16, C02-A6F1 and C04-N1400/B09 using one accounting boundary. All three carry the same unknown EO detector interface, SWIR detector/dewar interface, channel electronics and two focus functions; absent CAD and masses are left unquantified rather than assigned fake payload values. Optical mass is kept separate from complete payload mass. Coatings are treated as count sensitivities—0.810 for two neutral 0.90-reflectance mirrors without a splitter and 0.729 with a neutral 0.90 useful splitter port—not as supplier performance claims. NRE/tooling and recurring prices are separated at common comparison quote breaks of 1, 5, 10 and 25 units; those breaks are not a production forecast.

The normalized portfolio does not justify a sole C2 selection. C01 is the preferred C2 reference/fallback because it is the only compact point with complete physical polychromatic EO and SWIR prescriptions and a credible integrated fold. C02 remains a genuinely non-dominated challenger because its common fore-optic LOS, narrower front and single large optic could matter at payload level, but its annular f/1 primary, dichroic and real cameras/fold are coupled feasibility gates. C04 remains an economic hedge rather than a C2 optical-development lane: even perfect answers on donor clear aperture, coating and price do not change its preserved 273.4 L, 1.455 m geometry, 8.28 kg primary or materially worse inertia.

The blocking evidence before a sole-architecture commitment is: (1) common detector/dewar/window/electronics interfaces and flight environment; (2) a C01 concept CAD/thermal/boresight mass and gimbal estimate; (3) quoteable C02 annular-primary fabrication, metrology, support, finished mass and yield; (4) a finite dichroic transition allocated using source/QE/SNR information around 0.80 um; and (5) a real C02 three-element-camera pair in a physical folded package. Run 021 classifies each important unknown in `runs/run_021/unknowns.csv` and supplies concrete budgetary questions and nominal optical geometry in `runs/run_021/supplier_question_package.md`.

Next recommended action, only after new instruction: issue the common-basis C01/C02/C04 budgetary inquiries and obtain the detector/dewar/environment interfaces. If the C02 primary and dichroic replies remain credible, perform one bounded C02-A6F1 C2 run with real three-element EO/SWIR cameras, detector-window sensitivity and a physically folded two-branch package. In parallel, create the matching C01 concept CAD/thermal mass model. Down-select after these gates; do not open another architecture search or resume C04 optical optimization.

Run 021 used zero optimizer invocations, zero prescription attempts and zero substantial computational searches. No dependency was installed. `reference/` remained unread and historical runs/prescriptions were unchanged.

## Latest C04 cost/COTS evidence — Run 020

Run 020 extends the donor search to the current Orion 350-class primary hypothesis: catalogue 353 mm diameter, 1600 mm nominal focal length, 39 mm edge thickness and 8.28 kg, with current retail anchors around GBP 1,265–1,840 excluding VAT across grade/secondary choices. The reviewed pages do not state a guaranteed optical clear aperture. It is therefore modeled stopped to the required 311.371 mm pupil but remains conditional on a written >=311.371 mm usable-figure/edge-zone guarantee. The Edmund 317.5 mm example is excluded because its specified clear aperture is only 286 mm.

Five bounded variants tested the COTS/custom continuum. The retained N1400 SWIR point uses the fixed donor paraboloid, a catalogue 63 mm flat and two custom 50 mm spherical fused-silica/CaF2 elements. Dense replay gives 1930.50184 mm EFL, f/6.1999997, 9.22 um worst RMS and 93.55% sampled survival; the flat retains 12.19 mm field/beam margin. Infinity-to-800 m refocus is 4.648 mm. Coupled with preserved B09 EO evidence, the complete economic point has about 7.97 um EO / 9.22 um SWIR dense RMS and two focus mechanisms. No B-series prescription changed.

The optical result does not make a compact UAV payload. The common-bench screen is approximately 512 x 367 x 1455 mm (273.4 L), compared with 68.2 L for C01 B05-1/S16 and 79.7 L for normalized C02. A mid-path fold requires an approximately 262 mm tilted flat with a 3.11 kg solid proxy and increases the screen to 286.2 L, so it is parked. The mixed C04 optic estimate is about 8.84 kg, dominated by the donor primary, before cells/structure/electronics. Its vertical-axis uniform-box specific-inertia proxy is 0.1983 m2 per unit mass versus 0.0350/0.0503 for C01/C02, and the heavy primary at one end of a 1.455 m metering structure creates a material CG/gimbal burden.

The 258 mm EO/SWIR axis baseline produces 322.5 urad parallax at 800 m (93.5 EO or 62.3 SWIR pixels), but the full 5.304 x 4.243 m SWIR footprint remains inside the 11.305 x 8.280 m EO footprint. As for C01, range-aware registration and boresight stability matter more than common-scene loss at the specified distance.

The custom-secondary derivative shortens the screen to 183 L and reaches 10.08 um dense RMS, but needs a 158 mm K=-133 secondary, yields only 74.61% dense survival, and requires donor-primary perforation or a return-beam fold; it is parked because those changes undermine the COTS premise. The single-lens economy derivative retains the 273 L package and degrades to 24.26 um; it is parked. N1220 is parked because its nominal 82 mm flat is 2.31 mm undersized. N1300 remains a lower-priority placement checkpoint at 255 L/10.18 um/90% survival but has a 0.16 mm first-order miss after a capped solve.

Coating is explicitly separated from optical figure. Stock Hilux visible-coating claims are not extrapolated to 1.8 um. The preferred C04 branch is stock figured substrate plus a separately qualified SWIR coating; protected-gold products demonstrate a >96% average 700–2000 nm coating class, not availability or price on this donor. Stripping, figure re-verification, large-chamber availability, adhesion/environment testing, witness coupons and supplier dependence remain cost/schedule risks. A factory uncoated or qualified SWIR-coated donor would be preferable if quoted.

Pareto consequence: C04-N1400/B09 is CONDITIONALLY ACTIVE because it uniquely combines catalogue large powered figures, exact SWIR first order, useful image quality and only small custom spherical powered optics. Its potential large-optic procurement/NRE advantage is distinct enough to preserve despite much worse SWaP, but is not proved until clear aperture, coating, corrector quote, lot controls and lightweight-structure feasibility are supplied. C04 does not replace C01 or C02.

Run 020 used the normal five-attempt cap: 249 optimizer evaluations total, with four 60-evaluation bounded checkpoints and one nine-evaluation converged custom-secondary solve; the 800 m focus check used 16 scalar evaluations and two dense replays. Observed WSL process time was approximately 180 s plus reporting/integrity work. No dependency was installed. `reference/` remained unread and historical runs were unchanged.

Next recommended action, only after new instruction: one cross-portfolio C1 closure/RFQ-readiness run normalizing C01/C02/C04 detector-window, coating, structure and quantity assumptions and preparing exact supplier questions. If that gate justifies Stage C2 optical work, test one real three-element C02-A6F1 backend/folded package next. Do not further optimize C04 until its supplier gates are answered.
