...
def build(v):
    r2,k1,k2,r3,k3,z3,b,t,y,dt,ts=v
    ...
    l.surfaces.add(index=2,z=180,radius=-600,conic=k1,material='mirror',is_stop=True,aperture=ap(r_max=65))
...
def evaluate(v,n=7,details=False,wavelength=.55):
    ...
    for fx,fy in FIELDS:
        ray=l.trace(fx/.5019,fy/.5019,wavelength,num_rays=n,distribution='uniform')
        surfaces=[]
        for s in l.surfaces:
...
def residual(v,stage):
    try:
        ...
        r=np.r_[np.array(m['rms_um'])/5,(np.array(m['efl_xy_mm'])-EFL)/1.,
            np.maximum(0,KEEP-np.array(m['min_detector_clearance_mm']))/.3,
            np.maximum(0,.75-np.array(m['throughput']))/.02]
...
def main():
    ...
    res=least_squares(lambda x:residual(expand(x),stage),seed[active],bounds=(low[active],high[active]),
                      max_nfev=110,x_scale='jac',ftol=2e-5,diff_step=2e-5)
