...
STOP_DISTANCE=4.0  # Ahead of the entire parabolic sag, avoiding backward OPL.
...
class NewtonianPupil(RadialAperture):
    ...
    def contains(self,x,y):
        ...
        t=(STOP_DISTANCE-FLAT_DISTANCE-x)/(1+self.l);xf=x+t*self.l;yf=y+t*self.m
        clear_flat=xf*xf+yf*yf>(FLAT_MINOR/2)**2
        xs=x+(STOP_DISTANCE-FLAT_DISTANCE)*self.l;ys=y+(STOP_DISTANCE-FLAT_DISTANCE)*self.m
...
def build(x,focus=0,finite=False,folded=False):
    ...
    o.surfaces.add(index=1,thickness=STOP_DISTANCE,is_stop=True,aperture=NewtonianPupil())
    ...
    if folded:
        ...
        # Explicit reflection flag; negative material index alone does not
        # change Optiland's surface interaction mode.
        o.surfaces[3].interaction_model.is_reflective=True
