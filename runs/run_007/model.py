"""B04 physical-pupil screen, independently descended only from Track B Run 006.
...
def unscaled(x):
    ...
    return [[-2*f1,-sep,k,'mirror'],[r2,sep+.35*b,0,'mirror'],
            [1/c1,4,0,'N-BK7'],[1/c2,gap,0,'air'],[1/c3,3,0,'N-F2'],
            [1/c4,.65*b-7-gap+df,0,'air']]
...
def build(x,focus=0,finite=False):
    ...
    o=optic.Optic(name='B04 physical CDK, Run 007')
    ...
    o.surfaces.add(index=3,radius=surfaces[1][0]*s,thickness=sep,material='mirror',
                   aperture=RadialAperture(AP['secondary']))
    o.surfaces.add(index=4,thickness=.35*x[1]*s,aperture=RadialAperture(AP['hole']))
...
def main():
    x=np.array(json.loads((ROOT/'runs/run_006/configuration.json').read_text())['best'])
    if '--probe' in sys.argv:
        print(json.dumps(geometric(x),indent=2)[:700]); print(json.dumps(diffraction(x,(0,0),.55,64),indent=2)); return
    out=ROOT/'runs/run_007'; out.mkdir(exist_ok=False)
    ...
    # Same bounds as Run 006; EFL is enforced by uniform geometry scale and
    ...
    fit=least_squares(merit,x,bounds=bounds,x_scale='jac',max_nfev=75,ftol=1e-5,xtol=1e-6,gtol=1e-5)
    ...
    (out/'metadata.json').write_text(json.dumps(dict(track='B',candidate_ids=['B04'],run_id='007',parent_run='006',stage='screening',
        purpose='Restore EFL with physical pupils; assess diffraction and finite-conjugate behavior',start_utc=timestamp,
        end_utc=datetime.now(timezone.utc).isoformat(),wall_seconds=time.monotonic()-start,
        substantial_search_count=2,materially_distinct_attempt_count=1,candidate_stage_attempts_total=2,
        searches=['physical joint design','finite-conjugate scalar refocus'],plateau=False,
        assumptions=['Fixed mechanical aperture sizes','Ideal coatings/no spider or detailed baffles','Equal spectral weights for geometric merit',
